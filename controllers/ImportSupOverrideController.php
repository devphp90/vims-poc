...
class ImportSupOverrideController extends Controller
{
	/**
	...
	{
		$model=new ImportSupOverride;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['ImportSupOverride']))
		{
			$model->attributes=$_POST['ImportSupOverride'];
			if($model->save())
	...
	{
		$dataProvider=new CActiveDataProvider('ImportSupOverride');
	...
	public function actionAdmin()
	{
		...
		$model=new ImportSupOverride('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ImportSupOverride']))
			$model->attributes=$_GET['ImportSupOverride'];

    if ($status = $r->getQuery('status')) {
      ImportSupOverride::model()->updateAll(array('status' => $status == 'ON' ? 1 : 0));
    }
  ...
  public function actionStatusToggle($id)
  {
	...
	{
		$model=ImportSupOverride::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='import-sup-override-form')
		{
