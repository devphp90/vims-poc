...
class UbsPercentageRuleController extends Controller
{
	/**
	...
	{
		$model=new UbsPercentageRule;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['UbsPercentageRule']))
		{
			$model->attributes=$_POST['UbsPercentageRule'];
			if($model->save())
	...
	{
		$dataProvider=new CActiveDataProvider('UbsPercentageRule');
	...
	public function actionAdmin()
	{
		...
		$model=new UbsPercentageRule('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['UbsPercentageRule']))
			$model->attributes=$_GET['UbsPercentageRule'];

    if ($status = $r->getQuery('status')) {
      UbsPercentageRule::model()->updateAll(array('status' => $status == 'ON' ? 1 : 0));
    }
  ...
  public function actionStatusToggle($id)
  {
	...
	{
		$model=UbsPercentageRule::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ubs-percentage-rule-form')
		{
