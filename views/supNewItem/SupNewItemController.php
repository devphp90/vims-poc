    ...
    public function actionGetData()
    {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            if (count($dataProvider->data) > 0) {
                $data .= '{"total":' . count($dataProvider->data) . ',"rows":[';
                foreach ($dataProvider->data as $key => $result) {
                    ...
                    $data .= "{'matchby' : '{$matchBy}', 'match' : ''},";
                    
//                    $data .= "{'matchby' : '{$matchBy}', 'match' : '', 'diff' : '$diff', 'ubs_item_cost' : '$ubsItemCost', 'supp_item_price' : '$suppItemPrice', 'price_diff' : '$priceDiff',
